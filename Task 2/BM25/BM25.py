...
def BM25_pseudo():
    ...
    query_id = 1
    while query != "":
        score = calculate_pseudorelevance(query.split(), query_id)
        write_score(score, query_id)
        query_id += 1
        query = queries.readline()
...
def calculate_pseudorelevance(query_words, query_id):
    ...
    for term in INVERTED_INDEX:
        ...
        second_part_1 = beta*(1/len(RELEVANT_DOCS[str(query_id)]))

        sum_rel = 0
        sum_nonrel = 0
        if INVERTED_INDEX.has_key(term):
            for doc in RELEVANT_DOCS[str(query_id)]:
                if doc in INVERTED_INDEX[term]:
                    sum_rel += INVERTED_INDEX[term][doc]
            for doc in NONRELEVANT_DOCS[str(query_id)]:
                if doc in INVERTED_INDEX[term]:
                    sum_nonrel += INVERTED_INDEX[term][doc]

        second_part = second_part_1 * sum_rel

        third_part_1 = gamma*(1/len(NONRELEVANT_DOCS[str(query_id)]))

        third_part = third_part_1 * sum_nonrel
...
def write_score(score, query_id):
    score_file = open(BM25_PSEUDO_SCORE_DIR+"/Q" + str(query_id) +".txt", "w")
    builder = ""
...
def relevancy():
    global RELEVANT_DOCS, NONRELEVANT_DOCS
    filenames = []
    if os.path.exists(BM25_SCORE_DIR):
        file_names = os.listdir(BM25_SCORE_DIR)
        filenames = file_names
    for filename in filenames:
        bm25_query_file = open(BM25_SCORE_DIR+"/"+filename, "r")
        line_counter = 1
        for line in bm25_query_file.readlines():
            words_in_line = re.split(r"\t+", line)
            document = words_in_line[2]
            query_id = words_in_line[0]
            if line_counter > 5:
                if NONRELEVANT_DOCS.has_key(query_id):
                    NONRELEVANT_DOCS[query_id].append(document)
                else:
                    NONRELEVANT_DOCS[query_id] = [document]
            else:
                if RELEVANT_DOCS.has_key(query_id):
                    RELEVANT_DOCS[query_id].append(document)
                else:
                    RELEVANT_DOCS[query_id] = [document]
            line_counter += 1
