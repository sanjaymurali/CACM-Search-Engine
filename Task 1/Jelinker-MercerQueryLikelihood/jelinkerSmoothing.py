    ...
    for doc in FILENAMES_IN_CORPUS:
        ...
        for term in query_words:
            if term in INVERTED_INDEX and doc in INVERTED_INDEX[term]:
                firstTerm = (1-LAMBDAVAL)*(float(reduced_inverted_index[term][doc])/float(length_of_doc))
                #idf = math.log(NUMBER_OF_DOCS/nk[term])
                secondTerm = LAMBDAVAL*(get_term_frequency_from_inverted_index(reduced_inverted_index,term)/TOTAL_WORDS_IN_COLLECTION)
...
def get_term_frequency_from_inverted_index(reduced_inverted_index, term):
    count = 0
